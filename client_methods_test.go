...
func TestClientAllMethods(t *testing.T) {
	...
	mockServer.responses["initialize"] = InitializeResult{
		...
		Capabilities: ServerCapabilities{
			Tools:     &struct{ ListChanged bool `json:"listChanged,omitempty"` }{ListChanged: true},
			Resources: &struct{ Subscribe bool `json:"subscribe,omitempty"`; ListChanged bool `json:"listChanged,omitempty"` }{ListChanged: true},
			Prompts:   &struct{ ListChanged bool `json:"listChanged,omitempty"` }{ListChanged: true},
		},
	}
	...
	t.Run("CallTool", func(t *testing.T) {
		...
		if tc, ok := result.Content[0].(TextContent); !ok || tc.Text != "Tool result" {
