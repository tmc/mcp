...
func (b *toolCommandBuilder) command(used map[string]int) *cobra.Command {
	...
	cmd.Annotations = map[string]string{"tool-name": b.tool.Name}
	return cmd
}
...
func (b *toolCommandBuilder) run(cmd *cobra.Command) error {
	...
	raw, err := marshalArguments(args)
	if err != nil {
		return err
	}

	ctx, cancel := context.WithTimeout(cmd.Context(), b.timeout)
	defer cancel()
	result, err := b.backend.CallTool(ctx, mcp.CallToolRequest{
		Name:      b.tool.Name,
		Arguments: raw,
	})
	if err != nil {
	...
	return writeResult(cmd, result, b.raw)
}
...
func (s *jsonSchema) isObject() bool {
	return s != nil && s.kind() == "object"
}
...
func commandTransport(command string, stderr io.Writer) mcp.Transport {
	return mcpcli.CommandTransport(command, stderr)
}
