...
func main() {
	var (
		...
		namespace    = flag.String("namespace", "", "Register tunnel in namespace (ns://server/path)")
		namespaceURL = flag.String("ns-server", "", "Namespace server URL (if not in URI)")
	)
	flag.Parse()

	if flag.NArg() < 1 {
		log.Fatal("Usage: mcp-tunnel [options] -- command [args...]\n" +
			"   or: mcp-tunnel [options] -- ns://namespace/path")
	}

	cmd := flag.Arg(0)
	args := flag.Args()[1:]

	// Check if the command is a namespace URI
	if strings.HasPrefix(cmd, "ns://") {
		// Resolve from namespace
		nsServer, nsPath, err := ParseNamespaceURI(cmd)
		if err != nil {
			log.Fatalf("Invalid namespace URI: %v", err)
		}

		// Override namespace server if provided via flag
		if *namespaceURL != "" {
			nsServer = *namespaceURL
		}

		nsClient := NewNamespaceClient(nsServer)
		serviceInfo, err := nsClient.ResolveService(nsPath)
		if err != nil {
			log.Fatalf("Failed to resolve service: %v", err)
		}

		// Use resolved service info
		switch serviceInfo.Type {
		case "local":
			cmd = serviceInfo.Command
			args = serviceInfo.Args
			*transport = serviceInfo.Transport
		case "remote":
			cmd = serviceInfo.Address
			*transport = serviceInfo.Transport
		default:
			log.Fatalf("Unsupported service type: %s", serviceInfo.Type)
		}

		log.Printf("Resolved from namespace: %s", nsPath)
	}
	...
	// Register in namespace if requested
	if *namespace != "" {
		nsServer, nsPath, err := ParseNamespaceURI(*namespace)
		if err != nil {
			log.Printf("Invalid namespace registration URI: %v", err)
		} else {
			// Override namespace server if provided via flag
			if *namespaceURL != "" {
				nsServer = *namespaceURL
			...
			if err := nsClient.RegisterService(nsPath, serviceInfo); err != nil {
				log.Printf("Failed to register in namespace: %v", err)
			} else {
				log.Printf("Registered in namespace: %s", nsPath)
			}
		}
	}
