...
func TestListChangeNotifications(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var notified bool
			server.dispatch.Handle(tt.method, func(method string, params json.RawMessage) error {
				notified = true
				return nil
			})
			...
			if tt.caps.Tools != nil && tt.caps.Tools.ListChanged {
				err := server.dispatch.NotifyListChanged(tt.method)
				if err != nil {
					t.Fatal(err)
				}
			} else {
...
func TestNotifications(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			send: func(d *Dispatcher) error {
				return d.NotifyLoggingMessage(LogLevelInfo, "test", "hello")
			},
