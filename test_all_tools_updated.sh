...
# 2. Test mcpcat
echo "Testing mcpcat..."
OUTPUT=$(mcpcat test1.mcp 2>/dev/null)
...
OUTPUT=$(mcpcat -color=never test1.mcp 2>/dev/null)
...
OUTPUT=$(mcp-sort -strip test1.mcp 2>/dev/null)
...
# Test sorting
mcp-sort test1.mcp > sorted.mcp 2>/dev/null
...
OUTPUT=$(mcpdiff test1.mcp test2.mcp 2>/dev/null || true)
if echo "$OUTPUT" | grep -q 'new'; then
    echo "✓ mcpdiff works"
else
    echo "✗ mcpdiff failed to find differences"
    echo "Output was: $OUTPUT"
    exit 1
fi
...
OUTPUT=$(mcpdiff -compare shadow.mcp 2>/dev/null || true)
...
OUTPUT=$(mcpcat shadow.mcp 2>/dev/null)
...
OUTPUT=$(mcpspy -f spy.mcp cat input.txt 2>/dev/null)
...
echo '{"jsonrpc":"2.0","method":"test","id":1}' | mcpspy -f chain.mcp cat > /dev/null 2>&1
if [ -f chain.mcp ]; then
    OUTPUT=$(mcpcat chain.mcp 2>/dev/null | mcp-sort -strip 2>/dev/null)
    if [ ! -z "$OUTPUT" ]; then
