...
// MinimalOptions is an alias for MCPScripttestMinimalOptions for external API
type MinimalOptions = MCPScripttestMinimalOptions
...
// MCPScripttestMinimalOptions defines configuration options for minimal MCP scripttest
type MCPScripttestMinimalOptions struct {
	// Additional environment variables to inherit
	AdditionalEnvVars []string
	// Custom commands to add to the engine
	CustomCommands map[string]script.Cmd
...
func TestMinimal(t *testing.T, pattern string, opts ...*MCPScripttestMinimalOptions) {
	...
	if len(opts) > 0 && opts[0] != nil {
		minOpts := opts[0]
		fullOpts = &MCPScripttestOptions{
			AdditionalEnvVars:         minOpts.AdditionalEnvVars,
			CustomCommands:            minOpts.CustomCommands,
			CustomConditions:          minOpts.CustomConditions,
			IncludeDefaultMCPCommands: false, // Disable fancy commands
			RunDeadcodeCheck:          minOpts.RunDeadcodeCheck,
			DebugMode:                 minOpts.DebugMode,
			TimeoutConfig:             minOpts.TimeoutConfig,
		}
	} else {
		fullOpts = &MCPScripttestOptions{
			AdditionalEnvVars:         nil,
	...
	Test(t, pattern, fullOpts)
}

func NewMinimalEngine(opts ...*MCPScripttestMinimalOptions) *script.Engine {
	// Convert minimal options to full options with fancy commands disabled
	var fullOpts *MCPScripttestOptions
	if len(opts) > 0 && opts[0] != nil {
		minOpts := opts[0]
		fullOpts = &MCPScripttestOptions{
	...
	return NewEngine(fullOpts)
}

func DefaultMinimalOptions() *MCPScripttestMinimalOptions {
	return &MCPScripttestMinimalOptions{
		AdditionalEnvVars: nil,
		CustomCommands:    make(map[string]script.Cmd),
		CustomConditions:  make(map[string]script.Cond),
		RunDeadcodeCheck:  false,
