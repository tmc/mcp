// Package testcallgraph provides tools for analyzing test scripts to create call graphs
...
// BashExecution represents a bash script execution
...
// BashStitcher extends EnhancedStitcher with bash script analysis
type BashStitcher struct {
	*EnhancedStitcher
	BashScriptMap map[string][]BashExecution
}

// NewBashStitcher creates a new bash stitcher
...
func (s *BashStitcher) CreateBashCallGraph(filename string) []CallGraphEdge {
	...
	for _, bashExec := range bashExecs {
		...
		edges = append(edges, CallGraphEdge{
			From:     from,
			To:       to,
			EdgeType: "bash:" + bashExec.ExecutedBy,
			IsServer: false,
		})
	}
