...
func TestLogLevels(t *testing.T) {
	server := mcp.NewServer("log-test-server", "1.0.0",
		mcptestutil.WithTestLogger(t, slog.LevelDebug),
	)
	...
	handler := func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		logger := mcptestutil.TestLogger(t)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			server := mcp.NewServer("test-server", "1.0.0",
				mcptestutil.WithTestLogger(t, tt.level),
			)

			// Just create the pair to test logger setup
			pair, err := mcptestutil.NewServerClientPair(t, context.Background(), server)
			if err != nil {
				t.Fatal(err)
			}
			defer pair.Cleanup()
