...
// FileTestLogger wraps a testing.T to provide additional logging to a file
...
// NewFileTestLogger creates a new FileTestLogger that wraps t
func NewFileTestLogger(t *testing.T) *FileTestLogger {
	initLogFile()
	return &FileTestLogger{
		T:      t,
		prefix: t.Name() + ": ",
	}
}

// NewTestLogger is an alias for NewFileTestLogger for backward compatibility
func NewTestLogger(t *testing.T) *FileTestLogger {
	return NewFileTestLogger(t)
}

// Log logs to both the testing.T and the log file
func (tl *FileTestLogger) Log(args ...interface{}) {
...
func (tl *FileTestLogger) Logf(format string, args ...interface{}) {
	// First log to the testing.T
...
func (tl *FileTestLogger) Error(args ...interface{}) {
	// First log to the testing.T
	tl.T.Error(args...)
...
func (tl *FileTestLogger) Errorf(format string, args ...interface{}) {
	// First log to the testing.T
...
func (tl *FileTestLogger) Fatal(args ...interface{}) {
...
func (tl *FileTestLogger) Fatalf(format string, args ...interface{}) {
	// Log to the file first since Fatal will exit
	if logFile != nil {
		logMu.Lock()
...
func (tl *FileTestLogger) Skip(args ...interface{}) {
...
func (tl *FileTestLogger) Skipf(format string, args ...interface{}) {
	// Log to the file first since Skip will exit this test
	if logFile != nil {
		logMu.Lock()
...
func (tl *FileTestLogger) CapturePanicHandler(f func()) {
	defer func() {
		if r := recover(); r != nil {
