...
func TestTestLoggerError(t *testing.T) {
	...
	// Test that Error methods work by checking they don't panic
	// We'll use a special test that expects failures
	done := make(chan bool)
	go func() {
		defer func() {
			if r := recover(); r != nil {
				t.Errorf("Error method should not panic: %v", r)
			}
			done <- true
		}()

		// Create a dummy test that won't actually fail our test
		dummyT := &testing.T{}
		dummyLogger := &FileTestLogger{T: dummyT, prefix: "test: "}
		dummyLogger.Error("This is a test error")
		dummyLogger.Errorf("This is a formatted error: %s", "test")
	}()

	select {
	case <-done:
		// Success - methods didn't panic
...
func TestTestLoggerCapturePanicHandler(t *testing.T) {
	...
	// Test function that does panic - should capture it without failing
	panicCaptured := false
	done := make(chan bool)
	...
	go func() {
		...
		// Create a dummy test for panic testing
		dummyT := &testing.T{}
		dummyLogger := &FileTestLogger{T: dummyT, prefix: "test: "}

		// This should capture the panic and log it
		dummyLogger.CapturePanicHandler(func() {
			panic("test panic")
		})
	...
	case <-done:
		if !panicCaptured {
			t.Error("Panic handler test didn't complete")
		}
	case <-time.After(time.Second):
		t.Error("Test timed out")
	}
